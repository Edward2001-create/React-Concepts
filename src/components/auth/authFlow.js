...
export function flowReducer(state, action) {
    switch (action.type) {
        case "SWITCH_MODE": {
            // Flip between login and signup, resseting back to the first step.
        ...
        case "BACK": {
            ...
            if (state.step === "pasword") return { ...state, step: "email" };
        ...
        case "RESET":
            // Used on sign out: wipe the flow to a fresh login screen
...
export function friendlyError( err ) {
    switch(err?.code) {
        case "auth/invalied-credential":
        case "auth/wrong-password":
        case "auth/user-not-found":
            return "Email or password is incorrect.";
        case "auth/email-already-in-use":
            return "That email already has an account - try loggin in.";
